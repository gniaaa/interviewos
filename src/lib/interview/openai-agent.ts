...
If forceEvaluate is true, you must choose evaluate_answer, set toolName to evaluate_answer, set sessionStatus to evaluated, and include a non-null evaluation. Do not ask another follow-up when forceEvaluate is true.
...
All fields in the structured payload are required. For ask_follow_up and suggest_drill, set evaluation to null and sessionStatus to active. For evaluate_answer, set evaluation to a complete non-null rubric evaluation and sessionStatus to evaluated. Use nextFocusArea for the rubric area being coached; use null only if no rubric area applies.
